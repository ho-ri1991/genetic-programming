...
int main(){
    ...
    while(true) {
        std::cout << "Write tree file name or \'q\' and press Enter to exit." << std::endl;
        std::string input;
        std::cin >> input;
        if(input == "q") {
        ...
        try {
            ...
            auto stringToValueResult = [&stringToVariables, &prop = tree.getTreeProperty()](){
                ...
                std::transform(std::begin(prop.argumentTypes),
                               ...
                               [&stringToVariables](const utility::TypeInfo* x){
                                   if(!x) return utility::result::err<StringToVariable>("failed to load tree, argument type was not set properly");
                                   auto itr = stringToVariables.find(utility::TypeIndex(*x));
                                   return itr != std::end(stringToVariables)
                                          ? utility::result::ok(itr->second)
                                          : utility::result::err<StringToVariable>("failed to load tree, unknown argument type: " + x->name());
                               });
                return utility::result::sequence(std::move(ans));
            }();

            if(!anyToStringResult || !stringToValueResult) {
                std::cout << utility::result::sequence(anyToStringResult, stringToValueResult).errMessage() << std::endl;
                continue;
            }
            ...
            while(true) {
                std::cout << "input arguments separated by space to evaluate tree or press \'q\' and enter to quit evaluation" << std::endl;
                std::string in;
                std::getline(std::cin, in);
                if(in == "q") {
                    std::cout << "quit evaluation of the tree \"" << tree.getTreeProperty().name << "\"" << std::endl;
                    break;
                }
                ...
                auto argsResult = (std::size(argStrs) == tree.getArgumentNum()
                                   ...
                                   : utility::result::err<std::vector<std::string>>("the number of argument is invalid"))
                            ...
                            std::transform(std::begin(argStrings),
                                           ...
                                           [&n, &stringToValue](const std::string& str){
                                               return utility::result::tryFunction(
                                                       [&n, &stringToValue, &str](){return stringToValue[n++](str);},
                ...
                if(evaluatedValue.getEvaluationStatus() != utility::EvaluationStatus::ValueReturned) {
                    std::cout << "failed to evaluate tree: " << static_cast<int>(evaluatedValue.getEvaluationStatus()) << std::endl;
                    continue;
                }
                std::cout << "the evaluated value is: " << anyToString(evaluatedValue.getReturnValue()) << std::endl;
            }
        } catch (std::exception& ex) {
            std::cout << "failed to load tree, " << ex.what() << std::endl;
        }
    }
}