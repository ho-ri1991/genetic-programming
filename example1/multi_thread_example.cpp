...
constexpr std::size_t EVALUATION_CONCURRENCY = 4;
...
//            std::cout << "====" << std::to_string(count) << "th generation" << "====\n";
            std::vector<double> probabilities(std::size(population));
            static_assert(0 < EVALUATION_CONCURRENCY);
            auto taskGeneretor = [&population = population, &probabilities = probabilities, &ansArgList = problem1.ansArgList, &evaluator = evaluator, taskNum = EVALUATION_CONCURRENCY](std::size_t i) {
                assert(std::size(population) == std::size(probabilities));
                assert(0 < taskNum);
                assert(0 <= i && i < taskNum);
                std::size_t block = std::floor((double)std::size(population) / taskNum);
                auto limit = std::min(std::size(population), (i + 1) * block);
                return [&population = population, &probabilities = probabilities, &ansArgList = ansArgList, &evaluator = evaluator, limit = limit, idx = i, block = block]() {
                    for (int j = idx * block; j < limit; ++j) {
                        auto &tree = population[j];
                        double inverseFitness = 0;

                        //evaluation by each data set
                        for (std::size_t k = 0; k < std::size(ansArgList); ++k) {
                            const auto&
                            [answer, arguments] = ansArgList[k];
                            auto evaluatedValue = tree.evaluate(arguments);
                            inverseFitness += std::abs(evaluator(evaluatedValue, answer));
                        }
                        probabilities[j] = (1. / (1. + inverseFitness));
                    }
                };
            };
