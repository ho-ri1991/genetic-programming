...
BOOST_AUTO_TEST_SUITE(crossver)
    BOOST_AUTO_TEST_CASE(crossover) {
        ...
        {//case 3
            ...
            genetic_operations::DefaultLocalVariableAdapter<RND> localVariableAdapter(rnd);
            auto nodeSelector = [](const node::NodeInterface& root1, const node::NodeInterface& root2){
                return std::pair<const node::NodeInterface&, const node::NodeInterface&>(root1.getChild(0), root2);
            };
            ...
            BOOST_CHECK_EQUAL(cross1.getArgumentNum() ,1);
            BOOST_CHECK(cross1.getArgumentType(0) == utility::typeInfo<int>());
            BOOST_CHECK_EQUAL(cross1.getLocalVariableNum(), 2);
            BOOST_CHECK(cross1.getLocalVariableType(0) == utility::typeInfo<int>());
            BOOST_CHECK(cross1.getLocalVariableType(1) == utility::typeInfo<bool>());
        ...
        {//case 4
            ...
            genetic_operations::DefaultLocalVariableAdapter<RND> localVariableAdapter(rnd);
            auto nodeSelector = [](const node::NodeInterface& root1, const node::NodeInterface& root2){
                return std::pair<const node::NodeInterface&, const node::NodeInterface&>(root1, root2.getChild(2));
            };
        ...
        {//case 5
            ...
            genetic_operations::DefaultLocalVariableAdapter<RND> localVariableAdapter(rnd);
            auto nodeSelector = [](const node::NodeInterface& root1, const node::NodeInterface& root2){
                return std::pair<const node::NodeInterface&, const node::NodeInterface&>(root1, root2);
            };
    ...
    BOOST_AUTO_TEST_CASE(default_node_selector){
        constexpr std::size_t MAX_TREE_DEPTH = 3;
        RND rnd(0);
        auto [root1, prop1] = treeGen1();
        auto [root2, prop2] = treeGen2();

        genetic_operations::DefaultNodeSelector<RND> nodeSelector(rnd, MAX_TREE_DEPTH);

        for(int i = 0; i < 50; ++i){
            const auto& [node1, node2] = nodeSelector(*root1, *root2);
            BOOST_CHECK(node1.getReturnType() == node2.getReturnType());
            auto depth1 = tree_operations::getDepth(node1);
            auto height1 = tree_operations::getHeight(node1);
            auto depth2 = tree_operations::getDepth(node2);
            auto height2 = tree_operations::getHeight(node2);
            BOOST_CHECK_LE(height1 + depth2, MAX_TREE_DEPTH);
            BOOST_CHECK_LE(height2 + depth1, MAX_TREE_DEPTH);
        }
    }
